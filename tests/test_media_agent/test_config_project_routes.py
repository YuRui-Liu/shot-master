...
def test_project_overview_shape(tmp_path):
    ...
    assert body["project"]["shots_total"] == 4
    ...
    assert by_key["ideate"]["status"] == "done"
    assert by_key["storyboard"]["status"] == "done"
    assert by_key["imggen"]["status"] == "cur"     # production in_progress
    assert by_key["imggen"]["done"] == 2
    assert by_key["imggen"]["total"] == 4
    assert by_key["video"]["done"] == 1            # 1 集 video_done
