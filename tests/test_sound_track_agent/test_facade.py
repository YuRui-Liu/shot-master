...
def test_build_real_stages_reads_soundtrack_fade_out_from_cfg(tmp_path, monkeypatch):
    """facade._build_real_stages 把 cfg.soundtrack_fade_out 透传给 build_stages。"""
    captured = {"fade_out": "<unset>"}

    def fake_build_stages(**kwargs):
        captured["fade_out"] = kwargs.get("fade_out")
        from sound_track_agent.pipeline import Stages
        return Stages(
            tag_emotion=lambda seg, s: None,
            compose_prompt=lambda seg, s: "",
    ...
    _build_real_stages(_Cfg(), workflow_id="wf",
                       work_dir=tmp_path, global_style="x",
                       seeds_count=2, video_path=str(tmp_path / "ep.mp4"))
    assert captured["fade_out"] is True
